...
//                        .setAction("Action", null).show();
        ...
        listView.setAdapter(new CustomListAdapter(listData,this));
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> a, View v, int position, long id) {
                NewsItem newsData = (NewsItem) listView.getItemAtPosition(position);
                Toast.makeText(MainActivity.this, "Selected :" + " " + newsData.getHeading(), Toast.LENGTH_LONG).show();
            }
        });
    ...
    private ArrayList<NewsItem> getListData() {
        ...
        for (int i = 0; i < images.length; i++) {
            ...
            newsData.setHeading(heading[i]);
            newsData.setDescription(Description[i]);
        ...
        alertDialogBuilder.setCancelable(false)
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        //get name here and open new activity
